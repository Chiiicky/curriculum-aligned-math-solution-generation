...
from open_r1.rewards import (
    ...
    MathDifficulty,
    accuracy_reward,
    difficulty_gap_to_reward,
    format_reward,
    get_code_format_reward,
    get_cosine_scaled_reward,
    get_repetition_penalty_reward,
    get_reward_funcs,
    grade_value_to_difficulty_score,
    len_reward,
...
class TestRewards(unittest.TestCase):
    ...
    def test_difficulty_sigmoid_reward(self):
        self.assertAlmostEqual(difficulty_gap_to_reward(0.0, temperature=10.0), 0.5)
        self.assertAlmostEqual(difficulty_gap_to_reward(10.0, temperature=10.0), 0.731, places=3)

    def test_grade_value_to_normalized_difficulty_score(self):
        self.assertEqual(grade_value_to_difficulty_score(1.0), 0.0)
        self.assertAlmostEqual(grade_value_to_difficulty_score(12.5), 100.0)

    def test_cav_json_admits_only_gate_passing_nodes(self):
        calculator = MathDifficulty(kg_path=DEFAULT_KG_PATH)
        payload = {
            "used_nodes": [
                {
                    "node_id": "股定理与勾股定理逆定理",
                    "domain": "几何",
                    "subfield": "三角形",
                    "knowledge_point": "股定理与勾股定理逆定理",
                    "evidence": "由直角三角形两直角边平方和求斜边",
                    "role": "central",
                    "gate_scores": {"role": 0.95, "node_identity": 0.93, "stage": 0.91, "evidence": 0.96},
                ...
                },
                {
                    ...
                    "domain": "几何",
                    "subfield": "三角形",
                    "knowledge_point": "海伦公式",
                    "evidence": "也可以使用海伦公式，但这里不用",
                    "role": "incidental",
                    "gate_scores": {"role": 0.10, "node_identity": 0.90, "stage": 0.90, "evidence": 0.20},
                    "decision_source": "llm_fallback",
                    "confidence": 0.20,
                },
            ],
        ...
        admitted, audit_log = calculator.admitted_nodes_from_cav_payload(payload)

        self.assertEqual([node["node_id"] for node in admitted], ["股定理与勾股定理逆定理"])
        self.assertEqual(len(audit_log), 2)
        self.assertFalse(audit_log[1]["admitted"])

    def test_cav_response_parser_accepts_json_fence(self):
        calculator = MathDifficulty(kg_path=DEFAULT_KG_PATH)
        response = {"message": {"content": "```json\n{\"used_nodes\": [], \"unmatched_concepts\": [], \"requires_manual_review\": true}\n```"}}

        payload = calculator.parse_cav_response(response)

        self.assertTrue(payload["requires_manual_review"])

    def test_release_schema_graph_loads(self):
        release_graph = Path(__file__).resolve().parents[1] / "hkg" / "knowledge_base_graph57.json"
        calculator = MathDifficulty(kg_path=release_graph)

        self.assertEqual(len(calculator.hkg_node_records), 179)
        self.assertTrue(
            any(record["node_id"] == "HKG-GEO-TRI-PYTH" for record in calculator.hkg_node_records)
        )
